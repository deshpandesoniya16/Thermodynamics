...
class VendorShipAddrList extends Component {
...
if(JSON.parse(sessionStorage.getItem("Ship"))){

    let Shipaddr=JSON.parse(sessionStorage.getItem("Ship"))

    console.log("Ship",Shipaddr)

    this.setState({ship:Shipaddr})
}else{
    console.log("Nouser")
}
...
const{
   ...
   totalgst,
    ...
    tin,
    CST,
    panno,ECC,Excise,Collectorate,Ship,BillAddress
}=this.state
 ...
 let invoice = data.filter(i =>
    ...
    //console.log("invoice id",InvoiceData.invoice_id)
        ...
        return (
            ...
            <Link to="/VendorBillingInfo">
            <Icon name="arrow left"/>
            </Link>
            <Header size='large'>Shipping Address</Header> <br/>                   
                           ...
                           <List as='ul'>
                           {Ship.map(i=>(
    <List.Item as='li'>{i.ShipAddress}</List.Item>
))}
    </List>
...
export default VendorShipAddrList
