...
class EditVendorTransmute extends Component {
...
handlePan =e=>{
    ...
    setTimeout(() => {
        if (this.validatePan(this.state.panno)) {
            this.setState({ panno: this.state.panno ,message:""})
        } else {
            this.setState({ message: "Please Enter Valid Pan no." })
        }
    }, 1000);
    }
...
console.log(this.state.Collectorate)
  ...
  handleTransmute = ()=>{
    if(!this.state.tin || !this.state.CST || !this.state.panno || !this.state.gstno || !this.state.ECC || !this.state.Excise || !this.state.Collectorate){
        this.setState({open:true, msg:"All Fields Are Mandatory"})
    }else{

    sessionStorage.setItem("vendor_tin",this.state.tin)
    sessionStorage.setItem("vendor_cst",this.state.CST)
    sessionStorage.setItem("vendor_pan",this.state.panno)
    sessionStorage.setItem("vendor_gst",this.state.gstno)
    sessionStorage.setItem("vendor_ecc",this.state.ECC)
    sessionStorage.setItem("vendor_Excise",this.state.Excise)
    sessionStorage.setItem("vendor_collectorate",this.state.Collectorate)
    ...
    handleGst=e=>{
        ...
        setTimeout(() => {
            if (this.validateGSTIN(this.state.gstno)) {
                this.setState({ gstno: this.state.gstno ,message:""})
            } else {
                this.setState({ message: "Please Enter Valid GST No." })
            }
        }, 1000);
    }
...
const{
   ...
   totalgst,
    ...
    tin,
    CST,
    panno,ECC,Excise,Collectorate,clientData
}=this.state
    ...
    //console.log("invoice id",InvoiceData.invoice_id)
        ...
        <Side menuVisible={this.state.menuVisible}/>    
            ...
            <Sidebar.Pusher onClick={() => this.state.menuVisible && this.setState({ menuVisible: false })} dimmed={this.state.menuVisible}>
                ...
                <center>
                <Modal open={open} onClose={this.handleClose} className="alertOfFileds" basic size='small'>
                ...
                <Modal.Actions>
                  <Button basic color='red' inverted>
                    <Icon name='remove' onClick={()=>this.handleClose()} /> No
                           ...
                           <Grid.Column>
                           <div  style={{height:"860px",width:"139%",marginLeft: "-14%"}}>
                           <Card link fluid>
                       <Segment>
                       <center>
...
Disburse</label>
                     ...
                     <p style={{ color: "red" }}>{this.state.message}</p>
                           ...
                           {this.state.redirectToBilling && <Redirect to="/EditVendorBillingInfo" />}
                           </div>
...
export default EditVendorTransmute
