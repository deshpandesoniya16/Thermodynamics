...
import {
  ...
  clientList,
  addContract,
  addContractuploadFile,
  updateContract,
  deleteContract,
  AssignEmp,
  overHauling,
  onceIntwoMonth,
  breakdown, verticalBaseOit, verticalBaseBreak, verticalBaseOverhual
} from "../component/Api"
...
class AddContract extends Component {
  ...
  handleSubmit = async () => {
    ...
    if (!this.state.SelectedResultAssign.id) {
      this.setState({
        errorMsg: "Please Assign Employee",
        error: true,
        isopen: true
      })
      return
    ...
    if (!this.state.srNo) {
      this.setState({
        errorMsg: "Please Enter Serial No",
        error: true,
        isopen: true
      })
      return
    ...
    if (!this.state.fromDate) {
      this.setState({
        errorMsg: "Please Date Of commissioning",
        error: true,
        isopen: true
      })
      return
    ...
    if (!this.state.warrentyFromDate) {
      this.setState({
        errorMsg: "Please Select Warrenty From Date",
        error: true,
        isopen: true
      })
      return
    ...
    if (!this.state.contracttenureFromDate) {
      this.setState({
        errorMsg: "Please Select Tennure From Date",
        error: true,
        isopen: true
      })
      return
    ...
    if (!this.state.contracttenureToDate) {
      this.setState({
        errorMsg: "Please Select Tennure To Date",
        error: true,
        isopen: true
      })
      return
    ...
    if (!this.state.typeOfContract) {
      this.setState({
        errorMsg: "Please Select Type Of Contract",
        error: true,
        isopen: true
      })
      return
    ...
    if (!this.state.natureOfContract) {
      this.setState({
        errorMsg: "Please Select Nature Of Contract",
        error: true,
        isopen: true
      })
      return
    ...
    if (!this.state.ascStage) {
      this.setState({
        errorMsg: "Please Select Asc Stage",
        error: true,
        isopen: true
      })
      return
    ...
    if (!this.state.terms) {
      this.setState({
        errorMsg: "Please Enter Terms & Condition",
        error: true,
        isopen: true
      })
      return
    ...
    data.push({
      ...
      doc: moment(this.state.fromDate).format("YYYY-MM-DD"),
      // "Date_Of_Commision_To_Date":moment(this.state.toDate).format("DD-MM-YYYY"),
      warrantyFrom: moment(this.state.warrentyFromDate).format("YYYY-MM-DD"),
      tenureFrom: moment(this.state.contracttenureFromDate).format(
        "YYYY-MM-DD"
      ),
      tenureTo: moment(this.state.contracttenureToDate).format("YYYY-MM-DD"),
    ...
    console.log("data", data)
    await this.setState({ itemArray: data })

    if (this.state.itemArray.length > 0) {
      return this.setState({
        cannotaddContract: false,
  ...
  handleSubmitContract = () => {
    ...
    fetch(addContract, {
      ...
      body: JSON.stringify({
        ...
        aname: this.state.SelectedResultAssign.name,
        lastAction: cdate
      })
    })
      .then(data => {
      ...
      .then(data => {
        console.log("data", data)
        if (data.message == "contract added") {
          this.setState({
            activeItem: "upload",
            ContractId: data.contractNumber
          })
          window.scrollTo(0, 0)
        } else {
  ...
  addDocument = () => {
    ...
    fetch(addContractuploadFile, {
      ...
      .then(data => {
        console.log("data", data)
        if (data.message == "document added") {
          this.setState({ activeItem: "shedule" })
        } else {
          console.log("No Contract Added")
        }
      })
  ...
  handleUpdateGenrateShedule = () => {
    ...
    if (this.state.ContractId < 0) {
      this.setState({
        errorMsg: "Please add Contract to genrate contractId",
        error: true,
        isopen: true
      })
      return
    ...
    if (!this.state.oitName.length) {
      this.setState({
        errorMsg: "Please Select OIT",
        error: true,
        isopen: true
      })
      return
    ...
    if (!this.state.breakDownName.length) {
      this.setState({
        errorMsg: "Please Select break Down",
        error: true,
        isopen: true
      })
      return
    ...
    if (!this.state.overhaulingName.length) {
      this.setState({
        errorMsg: "Please Select Over Huling",
        error: true,
        isopen: true
      })
      return
    ...
    //   this.setState({
    //     errorMsg: "Please Enter Check Date",
    //     error: true,
    //     isopen: true
    //   })
    //   return
    ...
    fetch(updateContract, {
      ...
      body: JSON.stringify({
        custId: this.state.SelectedResult.id,
        oit: this.state.oitName,
        breakdown: this.state.breakDownName,
        overhauling: this.state.overhaulingName,
      ...
      <div>
        <PageContainer2>
          <div style={{ display: "flex", borderBottom: "2px solid #863577" }}>
            ...
            <HeadingDiv>
              <HeadingText>Add Contract Managment</HeadingText>
            </HeadingDiv>
          </div>
          <br />
          ...
          <HeadingDiv>
            ...
            >
              <Menu.Item
                ...
                disabled={!(activeItem === "Add Items")}
              />
              <Menu.Item
                name="Upload Files"
                active={activeItem === "upload"}
                onClick={this.handleItemClick}
                disabled={!(activeItem === "upload")}
              />
              <Menu.Item
                name="Generate Schedule"
                active={activeItem === "shedule"}
                onClick={this.handleItemClick}
                disabled={!(activeItem === "shedule")}
              />
            </Menu>
          </HeadingDiv>
                  ...
                  <Segment padded style={{ height: "100%" }}>
                    ...
                    >
                      <Button
                        ...
                        disabled={this.state.cannotaddContract}
                      >
                        Add Contract
                      </Button>
                    </ContentArea>
 ...
 */}
                    ...
                    <Button
                      color="blue"
                      onClick={() => this.handleUpdateGenrateShedule()}
                    >
                      Save
                    </Button>
                    {this.state.redirectToGodsView && (
