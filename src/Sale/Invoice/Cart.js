...
handleDisRs = e => {
  ...
  let Disprice = this.state.discountData.unitPrice - e.target.value;
  console.log("Dis price", Disprice);
  let totalAmount = Disprice * this.state.discountData.qty;
  console.log("total Amt", totalAmount);
  this.setState({ Disprice: Disprice });
...
removeDiscount = (size, item) => {
  ...
  this.state.totalAmountWithoutDis = parseInt(item.TaxableValue) + parseInt(item.discount)
...
this.state.discountData['discountPrice'] =this.state.totalAmountWithoutDis
//console.log("after remove discount",this.state.discountData)
this.setState({ })
  // item.discount
  // this.setState({ size, RD: true });
  // console.log("id of data is", item);
  // this.setState({ rdData: item, rdisrs:item.discount});
};

handleRdisRs = e=>{

  if (this.validateNumber(e.target.value)) {
    this.setState({ rdisrs: e.target.value });

    console.log(" remove Discounted price is ", e.target.value);
  }
  let RDisprice = this.state.discountData.unitPrice - e.target.value;
  console.log("Dis price", RDisprice);
  let totalAmountWithoutDis = RDisprice * this.state.discountData.qty;
  console.log("total Amt", totalAmountWithoutDis);
  this.setState({ RDisprice: RDisprice });
  this.setState({ totalAmountWithoutDis: Math.floor(totalAmountWithoutDis).toFixed(2) });

}


handleRemoveDiscount = () =>{
  this.state.rdData['discount'] = this.state.rdisrs
  this.state.rdData['discountPrice'] = this.state.RDisprice
  this.state.rdData['TaxableValue'] = this.state.totalAmountWithoutDis

  console.log("after add discount",this.state.rdData)
  this.setState({  rdisrs:"",RD:false})
    ...
    render(){
     ...
     // console.log("SelectedAsset is",SelectedAsset)
        ...
        return(
          ...
          {this.state.redirectToNext && <Redirect to="/PaymentBill" />}
        </PageContainer2>
                            ...
                            <Table.Body>
                              ...
                              <Grid relaxed columns={3} centered>
                                <Grid.Column style={{ width: "200px", marginRight: "20px" }}>
                                  <label>
                                    <font color="green">Discount in Rs: </font>
                                  </label>
                                </Grid.Column>
                              ...
                              <Grid relaxed columns={3} centered>
                                <Grid.Column>
                                  <label>
                                    <font color="green">Discount in Price:</font>
                                  </label>
                                </Grid.Column>
                                <Grid.Column>
                                  <label>
