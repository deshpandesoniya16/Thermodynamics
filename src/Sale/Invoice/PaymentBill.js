...
import {clientList,createInv} from "../../component/Api"
...
class PaymentBill extends Component {
    ...
    componentDidMount(){
        ...
        if(billaddr && shipaddr){
            this.setState({BillAddr:billaddr,ShipAddr:shipaddr,clientInfo:clientInfo,userInfo:userInfo})
        ...
        setTimeout(() => {
            ...
            this.setState({ orgTaxValue: this.state.taxvalue});
        ...
        setTimeout(() => {
            ...
            this.state.AssetCart.map(Item => {
                ...
                let itemAmount = Item.TaxableValue + tempGst
                ...
                // console.log("amt is in cdm",tamt)
                console.log("amt is in cdm",total)
           ...
           Item['Amount'] =  itemAmount
            ...
            // Saving Orignals 
            this.setState({ orgTotalGst: this.state.total_gst});
            this.setState({ orgCGST: this.state.CGST});
            this.setState({ orgSGST: this.state.SGST});
            this.setState({ orgTotalAmount: this.state.TotalAmount});
    ...
    calculateGst = (charges) =>{
        ...
        // if(charges > 0){
            let total_Amt = parseFloat(this.state.TotalAmount) +parseInt(this.state.other_charges)+parseInt(this.state.shipping_charges)
            console.log("Total Amount is",this.state.TotalAmount)
                this.setState({TotalAmount:total_Amt})
        // }else{

        //     this.setState({TotalAmount:this.state.orgTotalAmount})
        // }
...
// }
    ...
    handleShipping = e => {
        ...
        if (samount  > 0) {
          this.setState({ shipping_charges:samount });
            this.calculateGst();
        }else{
            e.target.value = 0
            ...
            this.calculateGst();
            this.setState({ shipping_charges:0 });
    ...
    handleOtherCharges = e => {
        ...
        let otherchrg = e.target.value
        if(otherchrg > 0){
          
          this.setState({other_charges:otherchrg})
          console.log(otherchrg)
          let total_Amt = parseFloat(this.state.TotalAmount)+parseInt(otherchrg)+this.state.shipping_charges
          console.log("Total Amount is in if",total_Amt)
              this.setState({TotalAmount:total_Amt})
        }else{
          otherchrg = 0
          this.setState({other_charges:otherchrg})
          ...
          console.log("Total Amount is in if",total_Amt)
          this.setState({TotalAmount:total_Amt})
            // console.log("ship",otherC)
            // this.setState({ other_charges: 0 });
            // this.calculateGst();
        }
    };
    ...
    genrateInvoice = () =>{
        ...
        this.state.AssetCart.map(Item => {
            Item['Amount'] =  this.state.itemAmount
    ...
    fetch(createInv, {
        ...
        body: JSON.stringify({
            customer_id:this.state.clientInfo.id,
            billingAddress:this.state.BillAddr,
            shippingAddress:this.state.ShipAddr,
            taxable_value:this.state.taxvalue,
        ...
        return data.json();
         ...
         let invoicID = data.invoiceId

         sessionStorage.setItem("invoiceID",invoicID)
        // window.location.reload()
         if(data.message=="Invoice Added succesfully"){
             this.setState({redirectToInvoice:true})
         }else{
            console.log("Somethimg went wrong")
            this.setState({msg:"Somethimg went wrong"})
         }
    ...
    render(){
        ...
        return(
          ...
          {this.state.redirectToInvoice && <Redirect to="/invoice" />}
