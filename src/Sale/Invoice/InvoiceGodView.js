...
import { InvoiceList } from "../../component/Api"
...
class InvoiceGodView extends Component {
  state = {
    menuVisible: false,
    startdate: "",
    dataList: [],
  ...
  invoiceGodsview = () => {
    fetch(InvoiceList, {
      ...
      .then(data => {
        ...
        console.log("Invoice data", data.records)
        if (data.records) {
          this.setState({ InvoiceData: data.records })
        } else {
  ...
  download = () => {
    ...
    this.JSONToCSVConvertor(this.state.InvoiceData, "Data", true)
  }
      ...
      <div>
        <PageContainer>
          <div style={{ display: "flex", borderBottom: "2px solid #863577" }}>
            ...
            <HeadingDiv>
              <HeadingText>Invoice God's View</HeadingText>
            </HeadingDiv>
          </div>
