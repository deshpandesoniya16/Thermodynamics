...
import {clientList,createQuotation} from "../../component/Api"
...
class QuotationBillpay extends Component {
...
// }
    ...
    handleShipping = e => {
        ...
        const re = /^[0-9\b]+$/;
        if ( re.test(samount) > 0) {
            this.state.shipping_charges=samount;
            this.calculateGst();
        }else{
            e.target.value = 0
            this.state.shipping_charges=0 
            let ship = e.target.value
            console.log("ship",ship)
           // this.state.TotalAmount = parseFloat(this.state.TotalAmount) - parseFloat (this.state.shipping_charges)
            this.calculateGst();

            // this.setState({total_gst :orgTotalGst });
            // this.setState({ CGST :orgCGST});
            // this.setState({ orgSGST: this.state.SGST});
            // this.setState({ orgTotalAmount: this.state.TotalAmount});
    ...
    handleOtherCharges = e => {
        ...
        const re = /^[0-9\b]+$/;
        if ( re.test(samount) > 0) {
            this.state.other_charges = samount 
            this.calculateGst();

        }else{
             this.state.other_charges = 0 ;
            e.target.value = 0
            let otherC = e.target.value
            console.log("ship",otherC)
            this.calculateGst();
           // this.setState.TotalAmount = parseFloat(this.state.TotalAmount) - parseFloat (this.state.other_charges)
    ...
    render(){
        ...
        return(
            ...
            <Segment style={{ width: "100vh" }}>
              <Form>
                <MainDivHolder>
                  ...
                  <MainDiv2>
                    <Box>
                      <span>Quotation Order Date :</span>
                    </Box>
                    <Box2>
                      <DatePicker
          ...
          {this.state.redirectToInvoice && <Redirect to="/quotation" />}
...
export default QuotationBillpay