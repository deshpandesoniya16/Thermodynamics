...
import { serviceList } from "../../component/Api"
...
class ServiceOrderGodView extends Component {
  state = {
    menuVisible: false,
    startdate: "",
    dataList: [],
  ...
  serviceGodsview = () => {
    fetch(serviceList, {
      ...
      .then(data => {
        ...
        console.log("service data", data.records)
        if (data.records) {
          this.setState({ serviceData: data.records })
        } else {
      ...
      <div>
        <PageContainer>
          <div style={{ display: "flex", borderBottom: "2px solid #863577" }}>
            ...
            <HeadingDiv>
              <HeadingText>Service Order God's View</HeadingText>
            </HeadingDiv>
          </div>
...
export default ServiceOrderGodView
