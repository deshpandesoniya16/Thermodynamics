...
import { purchaseOrderList } from "../../component/Api"
...
class PurchaseOrderGodView extends Component {
  state = {
    menuVisible: false,
    startdate: "",
    dataList: [],
  ...
  invoiceGodsview = () => {
    fetch(purchaseOrderList, {
      ...
      .then(data => {
        ...
        console.log("Purchase order data", data.records)
        if (data.records) {
          this.setState({ poData: data.records })
        } else {
      ...
      <div>
        <PageContainer>
          <div style={{ display: "flex", borderBottom: "2px solid #863577" }}>
            ...
            <HeadingDiv>
              <HeadingText>Purchase God's View</HeadingText>
            </HeadingDiv>
          </div>
...
export default PurchaseOrderGodView
